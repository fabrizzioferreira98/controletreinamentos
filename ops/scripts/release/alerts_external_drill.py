...
def main() -> int:
    ...
    webhook_url = ""
    if args.webhook_url:
        webhook_url = args.webhook_url.strip().lstrip("\ufeff")
    elif args.webhook_url_file:
        try:
            webhook_url = (open(args.webhook_url_file, "r", encoding="utf-8-sig").read() or "").strip().lstrip("\ufeff")
        ...
        webhook_url = (os.getenv(webhook_env_name, "") or "").strip().lstrip("\ufeff")
    if not webhook_url:
        print(
            json.dumps(
                {
