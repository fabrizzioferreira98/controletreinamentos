...
REQUIRED_EVIDENCE_ATTACHMENT_KEYS = EVIDENCE_ATTACHMENT_KEYS
...
def validate_checklist(
    ...
    *,
    ...
    for key in REQUIRED_EVIDENCE_ATTACHMENT_KEYS:
        value = (evidence_attachments.get(key, "") or "").strip()
        if not value:
            issues.append(f"evidence_attachment_missing:{key}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Valida checklist de regressao obrigatoria para release.")
    ...
    parser.add_argument(
        ...
        help="Resultados aceitos em Decisao/Resultado, separados por virgula.",
    )
    parser.add_argument(
        "--skip-head-commit-check",
        action="store_true",
        help="Nao compara Commit SHA do checklist com git HEAD (util em releases com checklist versionado).",
    )
