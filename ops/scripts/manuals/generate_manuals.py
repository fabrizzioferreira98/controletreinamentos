...
def generate_developer_manual(output_dir: Path):
    ...
    builder.bullets(
        [
            "backend/src/controle_treinamentos/compat/http_entrypoints/index.py: entrypoint HTTP residual de compatibilidade para inicialização segura; não é runtime oficial.",
            "backend/src/controle_treinamentos/compat/http_entrypoints/cron.py: endpoint HTTP residual protegido por CRON_SECRET; não é scheduler oficial.",
    ...
    builder.bullets(
        [
            ...
            "Trigger HTTP residual: backend/src/controle_treinamentos/compat/http_entrypoints/cron.py com Authorization Bearer CRON_SECRET. O agendamento oficial fica fora dessa superfície compat.",
