...
FIXED_COLUMNS = 10
HISTORICAL_IMPORT_CLASSIFICATION = "seed_import_historico"
HISTORICAL_IMPORT_APPLY_ACK_TOKEN = "import-tripulantes-csv-historico"
...
def main() -> int:
    ...
    parser.add_argument("--apply-ack", default="", help="Ack obrigatorio para importacao historica fora de dry-run")
    args = parser.parse_args()

    if not args.dry_run and (args.apply_ack or "").strip() != HISTORICAL_IMPORT_APPLY_ACK_TOKEN:
        print("Importacao historica congelada fora da trilha principal.")
        print(f"Rode com --dry-run ou informe --apply-ack {HISTORICAL_IMPORT_APPLY_ACK_TOKEN}.")
        return 1
