"""IMPLEMENTATION: compat residual de snapshot compativel/sync de tripulantes.

Comando explicitamente residual: backend/tools/compat_residual/sync_tripulantes_snapshot.py.
Nao e bootstrap, nao e migration estrutural e nao e trilha normal de operacao.
"""
...
DIRECT_ENTRY_NOTICE = (
    "Entrada direta residual: ops/scripts/database/sync_tripulantes_snapshot.py e implementacao; "
    "use backend/tools/compat_residual/sync_tripulantes_snapshot.py apenas por excecao controlada."
)
APPLY_ACK_TOKEN = "sync-tripulantes-snapshot-compat-residual"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--snapshot-dir",
        default=str(Path("archive") / "temp-backups" / "tripulantes_sync"),
    ...
    parser.add_argument(
        "--apply-ack",
        default="",
        help="Confirmacao explicita para a sincronizacao residual/aplicada.",
    )
    args = parser.parse_args(argv)

    if args.apply and (args.apply_ack or "").strip() != APPLY_ACK_TOKEN:
        print(
            json.dumps(
                {
                    "ok": False,
                    "classification": "compat_residual",
                    "error": "compat_apply_ack_required",
                    "message": (
                        "sync_tripulantes_snapshot.py sobrevive apenas como compat residual. "
                        "Nao use como fluxo normal."
                    ),
                    "required_ack": APPLY_ACK_TOKEN,
                },
                ensure_ascii=False,
                indent=2,
            )
