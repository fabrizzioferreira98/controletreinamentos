import {
  ...
  whatsappUrl,
  showFlash,
} from "./lib.js?v=20260408-143932";
import { renderShell } from "./shell.js?v=20260408-143932";

function renderTripulanteAvatar(item) {
  if (item.photo_url) {
    return `
      <div class="avatar avatar-sm">
        <img src="${escapeAttr(item.photo_url)}" alt="${escapeAttr(item.nome)}" loading="lazy" decoding="async">
      </div>
    `;
  }
...
export async function renderTripulanteFormPage(tripulanteId = null) {
  try {
    ...
    const photoUrl = tripulante?.photo_url || (tripulanteId ? `/api/v1/tripulantes/${tripulanteId}/photo` : "");
    const capabilities = capabilitySet();
