...
from .dashboard_cache import get_panel_cache, set_panel_cache
...
def fetch_competencias_disponiveis(db, *, limit: int = 18) -> list[str]:
    cache_key = f"produtividade:competencias:{limit}"
    ...
    rows = db.execute(
        ...
        SELECT competencia
        FROM (
            SELECT TO_CHAR(data_inicio, 'YYYY-MM') AS competencia
            FROM missoes_operacionais
            WHERE data_inicio IS NOT NULL
            UNION
            SELECT TO_CHAR(data_pernoite, 'YYYY-MM') AS competencia
            FROM pernoites_operacionais
            WHERE data_pernoite IS NOT NULL
            UNION
            SELECT competencia
    ...
    result = [row["competencia"] for row in rows if row.get("competencia")]
    set_panel_cache(cache_key, list(result))
    return result

def fetch_competencias_tripulante(db, *, tripulante_id: int, limit: int = 18) -> list[str]:
    rows = db.execute(
        """
        SELECT competencia
        FROM (
            SELECT TO_CHAR(m.data_inicio, 'YYYY-MM') AS competencia
            FROM missao_tripulantes mt
            JOIN missoes_operacionais m ON m.id = mt.missao_id
            WHERE mt.tripulante_id = %s
              AND m.data_inicio IS NOT NULL
            UNION
            SELECT TO_CHAR(p.data_pernoite, 'YYYY-MM') AS competencia
            FROM pernoites_operacionais p
            WHERE p.tripulante_id = %s
              AND p.data_pernoite IS NOT NULL
            UNION
            SELECT competencia
            FROM produtividade_adicionais_excepcionais
            WHERE tripulante_id = %s
              AND TRIM(COALESCE(competencia, '')) != ''
        ) base
        WHERE competencia IS NOT NULL
        GROUP BY competencia
        ORDER BY competencia DESC
        LIMIT %s
        """,
        (tripulante_id, tripulante_id, tripulante_id, limit),
    ).fetchall()
    return [row["competencia"] for row in rows if row.get("competencia")]

def fetch_produtividade_conferencias_map(db, *, competencia: str, tripulante_ids: list[int]) -> dict[int, dict]:
    ...
    rows = db.execute(
        ...
        WHERE pc.competencia = %s
          AND pc.tripulante_id = ANY(%s)
        """,
        (competencia, ids),
    ).fetchall()
    return {int(row["tripulante_id"]): dict(row) for row in rows}


def tripulante_exists(db, *, tripulante_id: int) -> bool:
    row = db.execute("SELECT id FROM tripulantes WHERE id = %s", (tripulante_id,)).fetchone()
    return bool(row)


def fetch_produtividade_conferencia(db, *, tripulante_id: int, competencia: str) -> dict | None:
    row = db.execute(
        """
        SELECT
            pc.tripulante_id,
...
def upsert_produtividade_conferencia(db, *, tripulante_id: int, competencia: str, user_id: int) -> None:
    db.execute(
        """
        ...
        """,
        (tripulante_id, competencia, int(user_id)),
    )


def delete_produtividade_conferencia(db, *, tripulante_id: int, competencia: str) -> None:
    db.execute(
