...
def create_tripulante_training_batch(payload: dict, *, criado_por: int) -> dict:
    ...
    try:
        for item in items:
            treinamento_id = create_training_program_record(
                db,
                data={
                    "tripulante_id": tripulante_id,
                    "tipo_treinamento_id": tipo_treinamento_id,
                    "segmento_teorico_id": item["segmento_id"],
                    "aeronave_modelo": template["aeronave_modelo"] or None,
...
def update_tripulante_training_record(payload: dict, *, treinamento_id: int) -> dict:
    ...
    try:
        update_training_program_record_row(
            db,
            treinamento_id=treinamento_id,
            data={
                "tripulante_id": tripulante_id,
                ...
                "aeronave_modelo": template["aeronave_modelo"] or None,
                "ctac_solo_horas": _optional_decimal_to_value(payload, "ctac_solo_horas", "Solo horas CTAC")
                if template["ctac_required"]
                else None,
                "ctac_voo_pic_sic_horas": _optional_decimal_to_value(payload, "ctac_voo_pic_sic_horas", "Voo PIC/SIC horas CTAC")
                if template["ctac_required"]
                else None,
                "ctac_voo_crew_horas": _optional_decimal_to_value(payload, "ctac_voo_crew_horas", "Voo CREW horas CTAC")
                if template["ctac_required"]
                else None,
                "data_realizacao": data_realizacao,
                "data_vencimento": data_vencimento,
