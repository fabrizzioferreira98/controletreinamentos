...
def _summarize(results: list[ScenarioResult]) -> dict:
    ...
    pct_5xx = [float(r.report.get("percent_5xx", 100.0) or 100.0) for r in results]
...
def _scenario_pass(summary: dict) -> bool:
    ...
    return (
        ...
        and float(((summary.get("availability_percent") or {}).get("worst", 0.0) or 0.0)) >= 99.0
        and float(((summary.get("latency_ms") or {}).get("p95_worst", 999999.0) or 999999.0)) <= 1200.0
        and float(((summary.get("percent_5xx") or {}).get("worst", 100.0) or 100.0)) <= 0.5
        and int(((summary.get("auth_failures") or {}).get("worst", 999999) or 999999)) == 0
        and int(((summary.get("permission_failures") or {}).get("worst", 999999) or 999999)) == 0
        and int(((summary.get("login_failures") or {}).get("worst", 999999) or 999999)) == 0
        and int(((summary.get("preflight_auth_failures") or {}).get("worst", 999999) or 999999)) == 0
    )
