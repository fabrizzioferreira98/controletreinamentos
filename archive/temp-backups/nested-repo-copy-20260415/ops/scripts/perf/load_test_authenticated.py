...
def _login(
    ...
    *,
    ...
    for attempt_idx in range(1, attempts + 1):
        ...
        if status != 200:
            last_reason = f"login_page_status:{status}"
            if status == 429:
...
def main() -> int:
    ...
    if args.endpoints.strip():
        ...
        endpoints = [
            ...
            "/produtividade?ordenacao=valor_final",
            "/produtividade?ordenacao=base&categoria=A",
    ...
    def _worker(worker_id: int):
        ...
        while time.time() < deadline:
            ...
            with lock:
                samples.append({"status": int(status), "duration_ms": duration_ms, "endpoint": endpoint})
