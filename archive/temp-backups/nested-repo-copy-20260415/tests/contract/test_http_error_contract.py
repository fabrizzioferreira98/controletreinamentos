...
def test_internal_error_trace_returns_event_for_known_request_id(monkeypatch):
    ...
    client = app.test_client()
    failed = client.get('/_test/internal-error-trace-source', follow_redirects=False)
...
def test_programmatic_dashboard_data_without_session_returns_json_401():
    app = create_app()
    client = app.test_client()

    response = client.get("/painel-tv/dados")
...
def test_programmatic_produtividade_tv_data_without_session_returns_json_401():
    app = create_app()
    client = app.test_client()

    response = client.get("/produtividade/painel-tv/dados")
...
def test_logout_programmatic_without_session_returns_json_401():
    app = create_app()
    client = app.test_client()
...
def test_logout_programmatic_authenticated_returns_json_200(monkeypatch):
    app = create_app()
    ...
    login_resp = client.post(
        "/login",
        data={"csrf_token": csrf, "login": "ativo_logout", "senha": "secret"},
        follow_redirects=False,
    )
...
def test_logout_programmatic_get_returns_405(monkeypatch):
    monkeypatch.setenv("SECRET_KEY", "dev-secret-for-test")
    app = create_app()
    client = app.test_client()
...
def test_logout_programmatic_post_with_invalid_csrf_still_terminates_session(monkeypatch):
    app = create_app()
    app.config["WTF_CSRF_ENABLED"] = False
    client = app.test_client()
...
def test_login_programmatic_invalid_credentials_returns_json_401(monkeypatch):
    ...
    response = client.post(
        "/login",
        data={"csrf_token": csrf, "login": "invalido", "senha": "x"},
        headers={"Accept": "application/json", "X-Requested-With": "XMLHttpRequest"},
    )
...
def test_login_programmatic_inactive_user_returns_json_403(monkeypatch):
    ...
    response = client.post(
        "/login",
        data={"csrf_token": csrf, "login": "inativo", "senha": "x"},
        headers={"Accept": "application/json", "X-Requested-With": "XMLHttpRequest"},
    )
...
def test_login_programmatic_success_returns_json_200(monkeypatch):
    ...
    response = client.post(
        "/login",
        data={"csrf_token": csrf, "login": "ativo", "senha": "secret"},
        headers={"Accept": "application/json", "X-Requested-With": "XMLHttpRequest"},
        follow_redirects=False,
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("GET", "/painel-tv/dados"),
        ("GET", "/produtividade/painel-tv/dados"),
        ("GET", "/bases/api/dados"),
