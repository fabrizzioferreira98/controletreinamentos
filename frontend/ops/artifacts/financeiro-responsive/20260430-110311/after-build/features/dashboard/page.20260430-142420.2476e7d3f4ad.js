...
import { BACKEND_LINKS } from "../../compat/backend-links.20260430-142420.db0529350261.js";

let dashboardSummarySnapshot = {};

function dashboardWeatherFallback(status = "unavailable") {
...
function renderDashboardHeader(options = {}) {
  ...
  const baseLabel = `${weather.icaoCode || "SBGO"} - ${weather.locationLabel || "Goi\u00e2nia"}`;
  ...
  return `
    <div class="page-header priority-page-header dashboard-page-header ui-page-header ui-surface">
      <div class="dashboard-header-main">
        <span class="dashboard-header-kicker">${escapeHtml(greetingPeriod)}, ${escapeHtml(displayName)}</span>
        <h1>Dashboard Operacional</h1>
        <p class="page-subtitle">Vis&atilde;o geral das opera&ccedil;&otilde;es, vencimentos e condi&ccedil;&otilde;es atuais.</p>
        <div class="dashboard-header-meta" aria-label="Atualiza&ccedil;&atilde;o da dashboard">
          <span>${escapeHtml(formatDashboardDateTime())}</span>
          <span>${escapeHtml(weather.updatedAtLabel || "Dados n\u00e3o atualizados")}</span>
        </div>
      </div>
      <aside class="dashboard-header-actions" aria-label="Condi&ccedil;&otilde;es operacionais atuais">
        <div class="dashboard-action-rail dashboard-weather-rail">
          <div class="dashboard-action-rail-head">
            <span class="dashboard-action-caption">Base Principal</span>
            <strong class="dashboard-weather-base">${escapeHtml(baseLabel)}</strong>
          </div>
          <div class="dashboard-weather-summary" title="${escapeAttr(metarTitle)}" aria-live="polite">
          ...
          <div class="dashboard-action-list dashboard-system-status-list">
            <span class="dashboard-system-badge dashboard-system-badge--${escapeAttr(weatherStatus)}">${escapeHtml(dashboardWeatherStatusLabel(weatherStatus))}</span>
          </div>
        </div>
      </aside>
    </div>
...
function adaptDashboardWeather(payload) {
  ...
  return {
    ...dashboardWeatherFallback(status),
    ...weather,
    icaoCode: String(weather.icaoCode || "SBGO").trim().toUpperCase(),
    locationLabel: String(weather.locationLabel || "Goi\u00e2nia").trim() || "Goi\u00e2nia",
    source: "AISWEB",
    status,
  };
}
...
export async function renderDashboardPage() {
  try {
    ...
    const [summaryResult, calendarResult, criticalResult, weatherResult] = await Promise.allSettled([
      api("/api/v1/dashboard/summary"),
      api("/api/v1/dashboard/calendar"),
      api("/api/v1/dashboard/critical-trainings"),
      api("/api/aisweb/met?icaoCode=SBGO"),
    ]);
    ...
    const weatherBlock = dashboardBlockFromResult(weatherResult, "Meteorologia", adaptDashboardWeather, dashboardWeatherFallback("error"));
    ...
    capabilities.dashboardWeather = weatherBlock.data;
    renderShell(
      `
        <div class="dashboard-page-shell priority-page-surface ui-page-shell ui-stack dashboard-reference-target dashboard-responsive-surface" data-dashboard-layout="responsive-operational">
        <div class="dashboard-top-cluster dashboard-fold-priority" data-dashboard-zone="above-fold">
