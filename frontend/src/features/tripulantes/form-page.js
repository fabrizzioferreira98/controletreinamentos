  ...
  try {
        ...
        <form id="tripulante-form" class="form-grid entity-form-grid tripulante-entity-form ui-form-grid" novalidate>
          ...
          <section class="full-width flags-section entity-form-section ui-surface ui-stack ui-form-section" data-tripulante-section="eligibility">
            ...
            <div class="form-grid form-grid-compact ui-form-grid ui-form-density-compact">
              <label>
                Validade SDEA/ICAO
                <input type="date" name="sdea_icao_validade" value="${escapeAttr(tripulante?.sdea_icao_validade || "")}">
                <span class="field-help ui-field-help">A produtividade paga SDEA/ICAO somente se a validade cobrir o ultimo dia da competencia.</span>
              </label>
              <label>
                Instrutor desde
                <input type="date" name="instrutor_inicio" value="${escapeAttr(tripulante?.instrutor_inicio || "")}">
                <span class="field-help ui-field-help">Inicio da designacao oficial de instrutor.</span>
              </label>
              <label>
                Instrutor ate
                <input type="date" name="instrutor_fim" value="${escapeAttr(tripulante?.instrutor_fim || "")}">
                <span class="field-help ui-field-help">Deixe vazio enquanto a designacao estiver vigente.</span>
              </label>
              <label>
                Checador desde
                <input type="date" name="checador_inicio" value="${escapeAttr(tripulante?.checador_inicio || "")}">
                <span class="field-help ui-field-help">Inicio da carta/designacao de checador.</span>
              </label>
              <label>
                Checador ate
                <input type="date" name="checador_fim" value="${escapeAttr(tripulante?.checador_fim || "")}">
                <span class="field-help ui-field-help">Deixe vazio enquanto a carta estiver vigente.</span>
              </label>
              <label class="full-width ui-form-field-long">
                Carta/designacao de checador
                <input type="text" name="checador_carta_designacao" value="${escapeAttr(tripulante?.checador_carta_designacao || "")}" placeholder="Ex.: Carta CHK-2026-001">
                <span class="field-help ui-field-help">O adicional de checador e limitado a uma carta/designacao vigente por competencia.</span>
              </label>
            </div>
          </section>
    ...
    function classifyPhotoSelection(file) {
      ...
      return {
        ok: true,
        cause: "ready",
        stateMessage: `${file.name} - ${formatFileSize(file.size)} - pre-visualizacao local; sera salva ao salvar o cadastro.`,
        formMessage: "",
        kind: "ready",
      };
    }
    ...
    async function attachSelectedPhotoToPayload(payload) {
      ...
      setPhotoUploadState(`${file.name} - preparando foto para salvar com o cadastro...`, "busy", "busy");
      try {
        payload.foto_base64 = await fileToDataUrl(file);
        return true;
      } catch (error) {
    ...
    function validateTripulanteForm() {
      ...
      const instrutorInicio = document.querySelector("input[name='instrutor_inicio']");
      const instrutorFim = document.querySelector("input[name='instrutor_fim']");
      const checadorInicio = document.querySelector("input[name='checador_inicio']");
      const checadorFim = document.querySelector("input[name='checador_fim']");
      const instrutorPeriodValid = setFieldFeedback(
        instrutorFim,
        instrutorInicio?.value && instrutorFim?.value && instrutorFim.value < instrutorInicio.value
          ? "Fim da designacao de instrutor nao pode ser anterior ao inicio."
          : "",
      );
      const checadorPeriodValid = setFieldFeedback(
        checadorFim,
        checadorInicio?.value && checadorFim?.value && checadorFim.value < checadorInicio.value
          ? "Fim da designacao de checador nao pode ser anterior ao inicio."
          : "",
      );
      validations.push(instrutorPeriodValid, checadorPeriodValid);
    ...
    const tripulanteDraft = wireCriticalFormDraftProtection({
      ...
      baselineFields: {
        ...
        sdea_icao_validade: tripulante?.sdea_icao_validade || "",
        instrutor_ativo: Boolean(tripulante?.instrutor_ativo),
        instrutor_inicio: tripulante?.instrutor_inicio || "",
        instrutor_fim: tripulante?.instrutor_fim || "",
        checador_ativo: Boolean(tripulante?.checador_ativo),
        checador_inicio: tripulante?.checador_inicio || "",
        checador_fim: tripulante?.checador_fim || "",
        checador_carta_designacao: tripulante?.checador_carta_designacao || "",
    ...
    document.getElementById("tripulante-form")?.addEventListener("submit", async (event) => {
      ...
      await withActionBusy(submitButton, "Salvando...", async () => {
        try {
          ...
          const photoAttached = await attachSelectedPhotoToPayload(payload);
          if (!photoAttached) return;
          ...
          const pendingPhotoFile = photoInput?.files?.[0];
          if (pendingPhotoFile && String(error?.code || "") === "tripulante_invalid_photo") {
            const failure = classifyPhotoUploadFailure(error, pendingPhotoFile);
            setPhotoUploadState(failure.stateMessage, failure.kind, "error");
          } else if (pendingPhotoFile) {
            setPhotoUploadState(`${pendingPhotoFile.name} - cadastro nao foi salvo; foto continua selecionada.`, "warning", "ready");
          }
