...
def test_extract_runtime_id_prefers_explicit_instance_header(tmp_path):
    ...
    module = _load_module("build_rollback_metadata_test", root / "ops" / "scripts" / "release" / "build_rollback_metadata.py")
...
def test_extract_runtime_id_falls_back_to_stable_header_fingerprint(tmp_path):
    root = Path(__file__).resolve().parents[2]
    module = _load_module(
        "build_rollback_metadata_hash_test",
        root / "ops" / "scripts" / "release" / "build_rollback_metadata.py",
    )
