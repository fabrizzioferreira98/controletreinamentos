...
def test_restore_ready_scope_rejects_theoretical_paths_even_outside_secure_env(tmp_path):
    assets_file = tmp_path / "assets.tar.gz"
    config_file = tmp_path / "config.tar.gz"
    _write_tar_gz(assets_file, {"static/styles.css": "body {}"})
    _write_tar_gz(config_file, {"env/prod.env": "DATABASE_URL=postgresql://local"})

    ok, issues = _validate_backup_scope(
        include_specs=["./backend/src/controle_treinamentos/static"],
        missing_include_specs=[],
        assets_file=assets_file,
        config_specs=["C:\\srv\\controle-treinamentos\\env"],
        missing_config_specs=[],
        config_file=config_file,
    ...
    assert ok is False
    assert any("uploads" in issue.lower() for issue in issues)
    assert any("caddy" in issue.lower() for issue in issues)
    assert any("tasks" in issue.lower() for issue in issues)


def test_backup_lock_is_stale_when_owner_pid_is_dead(monkeypatch):
    monkeypatch.setattr("backend.src.controle_treinamentos.infra.backup._process_exists", lambda pid: False)
    ...
    assert _backup_lock_is_stale(payload, stale_seconds=14400) is True
