...
def test_release_runtime_headers_are_emitted_when_configured(monkeypatch):
    monkeypatch.setenv("APP_ENV", "development")
    monkeypatch.setenv("APP_RELEASE_ID", "release-a")
    monkeypatch.setenv("APP_RELEASE_INSTANCE_ID", "instance-a")
    app = create_app()
    client = app.test_client()

    response = client.get("/login")

    assert response.headers["X-Release-ID"] == "release-a"
    assert response.headers["X-Release-Instance-ID"] == "instance-a"
