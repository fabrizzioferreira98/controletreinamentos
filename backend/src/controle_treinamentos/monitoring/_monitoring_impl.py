...
from ..infra.document_blobs import summarize_document_inventory
...
def _fetch_document_metadata_rows(db) -> tuple[list[dict], list[str]]:
    rows: list[dict] = []
    errors: list[str] = []
    queries = (
        """
    ...
    for query in queries:
        try:
            rows.extend(_as_dict_row(row) for row in db.execute(query).fetchall())
        except Exception as exc:
            errors.append(str(exc))
    return rows, errors


def _build_document_storage_risk_context(db) -> dict:
    rows, errors = _fetch_document_metadata_rows(db)
    if errors and not rows:
        return {
            "status_key": "unavailable",
            "message": "Nao foi possivel inventariar metadata/blob documental.",
            "problem_count": 1,
            "critical_count": 0,
            "warning_count": 1,
            "counts": {},
    ...
    try:
        summary = summarize_document_inventory(rows)
    except Exception as exc:
        return {
            "status_key": "unavailable",
            "message": "Falha ao classificar riscos de storage documental.",
            "problem_count": 1,
            "critical_count": 0,
            "warning_count": 1,
    ...
    if problem_count:
        message = (
            f"{problem_count} risco(s) documental(is): "
            f"{counts['metadata_without_blob']} metadata sem blob, "
            f"{counts['metadata_without_reference']} sem referencia, "
            f"{counts['unsupported_reference']} referencia invalida, "
            f"{counts['orphan_blobs']} blob(s) orfao(s)."
        )
    else:
        message = f"{counts['consistent']} referencia(s) metadata/blob sem risco classico detectado."
    return {
...
def _build_recent_error_signal(db) -> dict:
    window_minutes = max(1, _read_int_env("MONITORING_ERROR_ALERT_WINDOW_MINUTES", 15))
    error_warning_threshold = max(1, _read_int_env("MONITORING_ERROR_WARNING_THRESHOLD", 5))
    error_critical_threshold = max(
        error_warning_threshold + 1,
        _read_int_env("MONITORING_ERROR_CRITICAL_THRESHOLD", 15),
    )
    contract_warning_threshold = max(2, _read_int_env("MONITORING_CONTRACT_ERROR_WARNING_THRESHOLD", 3))
    contract_critical_threshold = max(
        contract_warning_threshold + 1,
        _read_int_env("MONITORING_CONTRACT_ERROR_CRITICAL_THRESHOLD", 8),
    )
    ...
    try:
        row = db.execute(
            ...
            WITH recent AS (
                ...
                WHERE captured_at >= CURRENT_TIMESTAMP - (%s * INTERVAL '1 minute')
            )
            SELECT
                ...
                COALESCE(
                    SUM(
                        CASE
                            WHEN haystack LIKE '%%contract%%'
                              OR haystack LIKE '%%contrato%%'
                              OR haystack LIKE '%%integration%%'
                              OR haystack LIKE '%%integracao%%'
                              OR haystack LIKE '%%schema%%'
                              OR haystack LIKE '%%pdf%%'
                              OR haystack LIKE '%%storage%%'
                              OR haystack LIKE '%%blob%%'
                              OR haystack LIKE '%%document%%'
                            THEN 1
                            ELSE 0
                        END
                    ),
                ...
                (SELECT request_id FROM recent ORDER BY captured_at DESC LIMIT 1) AS last_request_id,
                (SELECT path FROM recent ORDER BY captured_at DESC LIMIT 1) AS last_path,
                (SELECT code FROM recent ORDER BY captured_at DESC LIMIT 1) AS last_code
            FROM recent
            """,
            (window_minutes,),
        ).fetchone()
    except Exception as exc:
        message = "Nao foi possivel consultar request_error_events para diagnostico recente."
        return {
            "status_key": "unavailable",
            "message": message,
            "problem_count": 1,
            ...
            "alert": {
                "severity_key": "warning",
                "title": "Diagnostico de erros indisponivel",
                "message": message,
                "source": "diagnostico",
            },
        }

    data = _as_dict_row(row)
    total_errors = int(data.get("total_errors") or 0)
    server_errors = int(data.get("server_errors") or 0)
    contract_errors = int(data.get("contract_errors") or 0)
    last_request_id = (data.get("last_request_id") or "").strip()
    last_path = (data.get("last_path") or "").strip()
    last_code = (data.get("last_code") or "").strip()

    status_key = "operational"
    severity_key = ""
    title = ""
    if server_errors >= error_critical_threshold or contract_errors >= contract_critical_threshold:
        status_key = "degraded"
        severity_key = "critical"
        title = (
            "Falhas repetidas em contrato/integracao"
            if contract_errors >= contract_critical_threshold
            else "Surto recente de erros HTTP 5xx"
        )
    elif server_errors >= error_warning_threshold or contract_errors >= contract_warning_threshold:
        status_key = "attention"
        severity_key = "warning"
        title = (
            "Falhas recentes em contrato/integracao"
            if contract_errors >= contract_warning_threshold
            else "Erros HTTP 5xx em observacao"
        )

    last_context = ""
    if last_request_id:
        last_context = f" Ultimo request_id={last_request_id}"
        if last_code:
            last_context += f", code={last_code}"
        if last_path:
            last_context += f", path={last_path[:120]}"
        last_context += "."

    message = (
        f"Ultimos {window_minutes} min: {server_errors} erro(s) 5xx, "
        f"{contract_errors} contrato/integracao, {total_errors} erro(s) registrados."
        f"{last_context}"
    )
    alert = None
    if severity_key:
    ...
    return {
        "status_key": status_key,
        "message": message,
        "problem_count": max(server_errors, contract_errors) if status_key != "operational" else 0,
...
def _build_module_statuses(db, *, integrity: dict, storage: dict) -> tuple[list[dict], list[dict], dict]:
    ...
    try:
        from ..blueprints.auth.routes import auth_failure_surge_snapshot

        auth_failure_signal = auth_failure_surge_snapshot()
        auth_failure_status = auth_failure_signal["status_key"]
        auth_failure_count = int(auth_failure_signal.get("count") or 0)
        add_module(
            "Falhas de autenticacao",
            "Seguranca",
            auth_failure_status,
            auth_failure_signal["message"],
            auth_failure_count if auth_failure_status != "operational" else 0,
        )
        if auth_failure_status in {"degraded", "attention"}:
            alerts.append(
                {
                    "severity_key": "critical" if auth_failure_status == "degraded" else "warning",
                    "title": "Surto de falhas de autenticacao"
                    if auth_failure_status == "degraded"
                    else "Falhas de autenticacao em atencao",
                    "message": auth_failure_signal["message"],
                    "source": "seguranca",
                }
            )
    except Exception:
        add_module(
            ...
            "Seguranca",
            "unavailable",
            "Nao foi possivel ler a janela recente de falhas de autenticacao.",
            1,
        )
    ...
    add_module(
        ...
        (
            ...
            f"Pernoites: {pernoites_total or 0}."
        ),
    ...
    recent_error_signal = _build_recent_error_signal(db)
    add_module(
        "Falhas recentes",
        "Diagnostico",
        recent_error_signal["status_key"],
        recent_error_signal["message"],
        recent_error_signal["problem_count"],
    )
    if recent_error_signal.get("alert"):
        alerts.append(recent_error_signal["alert"])
    ...
    try:
        ...
        control_rows = db.execute(
            ...
            WHERE chave = ANY(%s)
            """,
            (list(SISTEMA_CONTROLE_NOTIFICATION_KEYS),),
        ).fetchall()
    ...
    try:
        from ..core.pdf_document_policy import (
            ...
            assert_pdf_document_payload,
            assert_pdf_visual_contract,
        )
        ...
        assert_pdf_visual_contract(AUDITORIA_EXPORT_PDF_POLICY)
        assert_pdf_document_payload(payload, policy=AUDITORIA_EXPORT_PDF_POLICY)
    except Exception:
        pdf_status = "degraded"
    ...
    document_risk = _build_document_storage_risk_context(db)
    add_module(
        "Consistencia metadata/blob",
        "Dados",
        document_risk["status_key"],
        document_risk["message"],
        document_risk["problem_count"],
    )
    if document_risk["status_key"] != "operational":
        alerts.append(
            {
                "severity_key": "critical" if document_risk["critical_count"] else "warning",
                "title": "Risco documental em storage",
                "message": document_risk["message"],
                "source": "documentos",
            }
        )
...
def collect_system_monitoring_snapshot(db) -> dict:
    ...
    auth_failure_module = module_map.get("Falhas de autenticacao") or next(
        (item for item in modules if "autentica" in (item.get("name", "").lower())),
        None,
    )
    recent_error_module = module_map.get("Falhas recentes")
    operational_focus = [
        {
        ...
        },
        {
            ...
            **(
                auth_failure_module
                if auth_failure_module
                else {"message": "Sem diagnostico da janela de falhas de autenticacao.", **_status_payload("unavailable")}
            ),
        },
        {
            ...
            **(
                recent_error_module
                if recent_error_module
                else {"message": "Sem diagnostico dos erros recentes.", **_status_payload("unavailable")}
            ),
        },
        {
