...
def fetch_training_program_tripulantes(db, *, base: str | None = None) -> list[dict]:
    base_filter = (base or "").strip()
    base_join = ""
    clauses = ["c.ativo = 1"]
    params: list = []
    if base_filter:
        base_join = "LEFT JOIN bases pb ON pb.id = p.base_id"
...
def fetch_training_program_record_list(
    ...
    *,
    ...
    tipo_treinamento_id: int | None = None,
    aeronave_modelo_snapshot: str | None = None,
    base: str | None = None,
) -> list[dict]:
    ...
    if tipo_treinamento_id is not None:
        ...
        params.append(int(tipo_treinamento_id))
    if aeronave_modelo_snapshot:
        clauses.append("LOWER(COALESCE(t.aeronave_modelo, '')) = LOWER(%s)")
        params.append(str(aeronave_modelo_snapshot))
    if base_filter:
        base_join = "LEFT JOIN bases pb ON pb.id = p.base_id"
        clauses.append("LOWER(TRIM(COALESCE(pb.nome, ''))) = LOWER(%s)")
        params.append(base_filter)
    ...
    rows = db.execute(
        ...
        WITH ctac_refs AS (
            SELECT DISTINCT
                hv.tipo_treinamento_id,
                LOWER(hv.aeronave_modelo) AS aeronave_modelo_key
            FROM horas_voo_aeronave hv
            WHERE hv.ativo = 1
              AND POSITION('conforme ctac' IN LOWER(COALESCE(hv.observacao, ''))) > 0
        )
        SELECT
            t.id,
            t.tripulante_id,
            ...
            t.aeronave_modelo AS aeronave_modelo_snapshot,
            t.ctac_solo_horas,
            ...
            (
                ...
                WHERE a.treinamento_id = t.id
                  AND a.status = 'ativo'
            ) AS total_anexos,
            (ctac_refs.tipo_treinamento_id IS NOT NULL) AS ctac_required
        FROM treinamentos t
        ...
        LEFT JOIN ctac_refs
            ON ctac_refs.tipo_treinamento_id = t.tipo_treinamento_id
           AND ctac_refs.aeronave_modelo_key = LOWER(COALESCE(t.aeronave_modelo, ''))
        {where}
        ORDER BY
    ...
    for row in rows:
        item = dict(row)
        item["status_calculado"] = calculate_training_status(item.get("data_vencimento"), reference=today)
        item["ctac_required"] = bool(item.get("ctac_required"))
        items.append(item)
    return items
