...
from .queries import fetch_training_page
...
def build_training_filters(*, tripulante="", equipamento="", tipo="", status="", periodo=""):
    ...
    if equipamento:
        ...
        clauses.append("t.equipamento_id = %s")
...
def get_treinamentos_summary(db, *, where_clause: str = "", params=(), today) -> dict:
    row = db.execute(
        f"""
        SELECT
            COUNT(*) AS total,
...
def list_treinamentos_ssr_page(db, *, where_clause: str = "", params=(), limit: int, offset: int) -> list[dict]:
    return fetch_training_page(
        db,
        where_clause,
        tuple(params),
        limit=limit,
...
def get_treinamento_for_edit(db, *, treinamento_id: int) -> dict | None:
    row = db.execute("SELECT * FROM treinamentos WHERE id = %s", (treinamento_id,)).fetchone()
...
def fetch_treinamento_attachments(db, *, treinamento_id: int, include_removed: bool = False):
    status_filter = "" if include_removed else "AND COALESCE(NULLIF(TRIM(a.status), ''), 'ativo') <> 'removido'"
    rows = db.execute(
        f"""
        SELECT
            a.id,
...
def create_treinamento_attachment_record(db, *, treinamento_id: int, parsed: dict, enviado_por: int) -> int:
    row = db.execute(
        ...
        (
            ...
            parsed["storage_ref"],
            database_blob_for_persistence(parsed, allow_legacy_database_blob=False),
            parsed["arquivo_hash"],
            enviado_por,
        ),
    ).fetchone()
...
def find_active_training_attachment_duplicate_hash(db, *, treinamento_id: int, arquivo_hash: str):
    return db.execute(
        """
        ...
        WHERE treinamento_id = %s
          AND arquivo_hash = %s
          AND COALESCE(NULLIF(TRIM(status), ''), 'ativo') <> 'removido'
        LIMIT 1
        """,
        (treinamento_id, arquivo_hash),
    ).fetchone()
...
def delete_treinamento_attachment_record(
    db,
    *,
    treinamento_id: int,
    anexo_id: int,
    removido_por: int | None = None,
) -> None:
    db.execute(
        """
        ...
        WHERE id = %s
          ...
          AND COALESCE(NULLIF(TRIM(status), ''), 'ativo') <> 'removido'
        """,
        (removido_por, "Removido manualmente.", anexo_id, treinamento_id),
    )
