...
def _register_legacy_src_app_alias() -> None:
    legacy_root = sys.modules.get("src")
    if legacy_root is None:
        legacy_root = types.ModuleType("src")
        legacy_root.__path__ = []  # type: ignore[attr-defined]
        sys.modules["src"] = legacy_root
    sys.modules["src.app"] = sys.modules[__name__]
    setattr(legacy_root, "app", sys.modules[__name__])


_register_legacy_src_app_alias()
...
def create_app() -> Flask:
    ...
    release_id = (os.getenv("APP_RELEASE_ID", "") or os.getenv("CONTROL_TREINAMENTOS_RELEASE_ID", "") or "").strip()
    release_instance_id = (
        os.getenv("APP_RELEASE_INSTANCE_ID", "")
        or os.getenv("CONTROL_TREINAMENTOS_RELEASE_INSTANCE_ID", "")
        or release_id
    ).strip()
    ...
    app.config.update(
        ...
        APP_RELEASE_INSTANCE_ID=release_instance_id,
    ...
    app.logger.info(
        ...
        extra={
            ...
            "instance_path": str(instance_path),
            "sentry_configured": bool(sentry_sdk and os.getenv("SENTRY_DSN")),
        },
    )
    ...
    def internal_metrics():
        ...
        from .core.metrics import (
            ...
            refresh_environment_signal,
            refresh_runtime_resource_metrics,
            set_background_job_queue_snapshot,
        )

        refresh_environment_signal(app_env=str(app.config.get("APP_ENV") or "development"))
        refresh_runtime_resource_metrics(instance_path=app.instance_path)
        try:
            from .db import get_db
            from .infra.jobs import collect_job_queue_snapshot

            set_background_job_queue_snapshot(collect_job_queue_snapshot(get_db()))
        except Exception:
            record_critical_flow_failure("metrics_refresh", "job_queue_unavailable")
            app.logger.exception("Metrics refresh failed while collecting job queue snapshot.")
    ...
    csrf.exempt(auth_routes.logout)
    csrf.exempt(auth_routes.api_session_logout)
    ...
    app.logger.info(
        ...
        extra={
            ...
            "app_env": app_env,
            "release_id": release_id,
            "blueprints": ["dashboard", "auth", "cadastros", "operacoes", "financeiro", "admin", "relatorios"],
