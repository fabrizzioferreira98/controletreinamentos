...
from ...core.pdf_document_policy import AUDITORIA_EXPORT_PDF_POLICY, build_pdf_document_response
...
from ...mailer import generate_notification_payload, get_smtp_config, send_test_email, validate_notification_dispatch_readiness
...
_NOTIFICATION_PAYLOAD_CACHE_PREFIX = "notificacoes:payload:"
_NOTIFICATION_PAYLOAD_CACHE_TTL_SECONDS = 60
_USUARIOS_LIST_CACHE_PREFIX = "usuarios:list:"
_USUARIOS_LIST_CACHE_TTL_SECONDS = 60


def _notification_payload_cache_key(*, preview_enabled: bool) -> str:
    return f"{_NOTIFICATION_PAYLOAD_CACHE_PREFIX}{int(bool(preview_enabled))}"


def _clear_notification_payload_cache() -> None:
    clear_panel_cache(_NOTIFICATION_PAYLOAD_CACHE_PREFIX, invalidate_global=False)


def _clear_usuarios_list_cache() -> None:
    clear_panel_cache(_USUARIOS_LIST_CACHE_PREFIX, invalidate_global=False)
...
def usuarios_list():
    db = get_db()
    cache_key = f"{_USUARIOS_LIST_CACHE_PREFIX}{request.query_string.decode('utf-8', errors='ignore')}"
    cached_context = get_panel_cache(cache_key)
    if cached_context is not None:
        return render_template("usuarios_list.html", **cached_context)
    ...
    set_panel_cache(cache_key, context, ttl_seconds=_USUARIOS_LIST_CACHE_TTL_SECONDS)
    return render_template("usuarios_list.html", **context)
...
def auditoria_export_pdf():
    ...
    audit_document_generation(
        db,
        policy=AUDITORIA_EXPORT_PDF_POLICY,
        filename=filename,
        filters={
            "entidade": entidade or "",
            "acao": acao or "",
        ...
        details={"rows_count": len(audit_rows)},
        commit=True,
    )
...
def notificacoes_list():
    try:
        ...
        try:
            payload_cache_key = _notification_payload_cache_key(preview_enabled=preview_enabled)
            cached_payload = get_panel_cache(payload_cache_key)
            if cached_payload is None:
                cached_payload = generate_notification_payload(
                ...
                set_panel_cache(
                    payload_cache_key,
                    cached_payload,
                    ttl_seconds=_NOTIFICATION_PAYLOAD_CACHE_TTL_SECONDS,
                )
        ...
        controle = db.execute(
            "SELECT chave, valor FROM sistema_controle WHERE chave = ANY(%s)",
            (list(SISTEMA_CONTROLE_NOTIFICATION_KEYS[:2]),),
        ).fetchall()
...
def notificacoes_new():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            db.commit()
            _clear_notification_payload_cache()
        ...
        try:
            ...
            audit_event(
                ...
                novo={
                    ...
                    "source": "manual_notifications",
                    "idempotency_key": enqueue_result.idempotency_key,
                },
                observacao="Solicitacao manual de disparo de notificacoes pelo painel administrativo.",
            )
            db.commit()
        except Exception:
            rollback_db(db)
            ...
            try:
                ...
                audit_event(
                    ...
                    novo={
                        ...
                        "status": enqueue_result.status,
                        "created": enqueue_result.created,
                        "source": "manual_notifications_retry",
                        "idempotency_key": enqueue_result.idempotency_key,
                    },
                    observacao="Reenfileiramento manual de notificacoes apos falha anterior.",
                )
                db.commit()
            except Exception:
                rollback_db(db)
...
@admin_bp.route("/notificacoes-email/teste", methods=["POST"])
@permission_required("notificacoes:edit")
def notificacoes_test_send():
    try:
        try:
            readiness = validate_notification_dispatch_readiness()
        except Exception:
            current_app.logger.exception("Falha ao validar pré-requisitos para envio de e-mail de teste.")
            flash(
                "Não foi possível validar os pré-requisitos do teste agora. Tente novamente em instantes.",
                "error",
            )
            return redirect(url_for("admin.notificacoes_list"))

        if int(readiness.get("recipients_count", 0) or 0) <= 0:
            flash("Nenhum destinatário ativo configurado para o envio de teste.", "error")
            return redirect(url_for("admin.notificacoes_list"))
        ...
        if not readiness.get("email_ready"):
            ...
            if (readiness.get("provider") or "").strip().lower() == "resend":
                flash(
                    "Resend não configurado. Defina RESEND_API_KEY e RESEND_FROM antes do envio de teste."
                    f"{missing_details}",
                    "error",
                )
            else:
                flash(
                    "SMTP não configurado. Defina SMTP_HOST, SMTP_USER e SMTP_PASSWORD antes do envio de teste."
                    f"{missing_details}",
                    "error",
                )
            return redirect(url_for("admin.notificacoes_list"))
        ...
        requested_by = (
            (getattr(current_user, "nome", None) or "").strip()
            or (getattr(current_user, "login", None) or "").strip()
            or f"usuario#{int(current_user.id)}"
        )
        ...
        if result.sent:
            flash("E-mail de teste enviado com sucesso para os destinatários ativos.", "success")
            return redirect(url_for("admin.notificacoes_list"))
        if result.reason == "no_recipients":
            flash("Nenhum destinatário ativo configurado para o envio de teste.", "error")
            return redirect(url_for("admin.notificacoes_list"))
        if result.reason == "smtp_not_configured":
            flash(result.error or "Configuração SMTP incompleta para envio de teste.", "error")
            return redirect(url_for("admin.notificacoes_list"))
        error_text = result.error or "Erro não detalhado."
        flash(f"Falha ao enviar e-mail de teste: {error_text}", "error")
        return redirect(url_for("admin.notificacoes_list"))
    except Exception:
        current_app.logger.exception("Falha inesperada no envio de e-mail de teste.")
        flash("Não foi possível concluir o envio de teste neste momento. Tente novamente.", "error")
        return redirect(url_for("dashboard.dashboard"))
